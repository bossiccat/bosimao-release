"""Commercial voice status and websocket stream routes."""
...
from .routes_voice_security_context import (
    SecuredVoiceDeps, client_ip, record_session_event, resolve_status_principal,
)
...
def build_status_stream_router(deps: SecuredVoiceDeps) -> APIRouter:
    ...
    async def voice_status(request: Request):
        ...
        allowed, _retry = deps.limiter.check(
            principal.subject_id, client_ip(request), "voice:status"
        )
        if not allowed:
        ...
        data = {
            ...
            "up_frame_count": 0, "up_bytes": 0,
            "down_frame_count": 0, "down_bytes": 0,
            "first_remote_audio_ts": None, "first_nonzero_playback_ts": None,
            "queue_depth": 0, "queue_high_watermark": 0, "queue_drops": 0,
            "backpressure_events": 0, "reconnects": 0, "error_code": None,
        }
    ...
    async def voice_stream(ws: WebSocket) -> None:
        ...
        try:
            ...
            await ws.close(code=WS_CLOSE_AUTH)
            return
        if not deps.nonces.consume(principal, ws.headers.get("x-request-nonce", "")):
        ...
        try:
            while True:
                ...
                if device is None or device.status == "revoked" or device.revoked_at is not None:
