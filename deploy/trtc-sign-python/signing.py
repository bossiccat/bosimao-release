...
class TrtcSignService:
    ...
    def list_pending(self) -> list[dict]:
        """PC 轮询（v0.6.1）：返回全部未消费会话意图列表（PC 不知道手机 device_id，
        需枚举；保鲜期内的才返回，过期清理）——修复 PC 无法发现手机意图的缺陷"""
        now = time.time()
        out: list[dict] = []
        for device_id, intent in list(self._intents.items()):
            if now - intent["ts"] > INTENT_FRESH_S:
                self._intents.pop(device_id, None)
                continue
            out.append({"device_id": device_id, "room_id": intent["room_id"], "ts": intent["ts"]})
        return out
