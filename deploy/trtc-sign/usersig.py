...
注意：本文件为云函数 trtc-sign 的独立副本，与
backend/app/voice/usersig.py 保持字节级一致（Phase A 已与官方字节级对照）。
云函数环境无 backend 包依赖，故复制一份到本目录；若后续修改请双端同步。
...
SecretKey 只从环境变量 TRTC_SECRETKEY 注入（config.py），本模块不落任何密钥默认值。
"""
...
def gen_user_sig(sdk_app_id: int, secret_key: str, user_id: str, expire_s: int = 600) -> str:
    ...
    Args:
        ...
        secret_key: TRTC 控制台 SecretKey（仅环境变量，禁入库/日志）
