...
**一句话结论**：wave2 把「卡断 / 静音 / 误打断 / 采集削波」四类故障的代码级根因全部定位并修复、每一条都有 RED→GREEN 证据，但**无线 ADB 未打通 → 09-07 当天零真机取证**，商用门禁三条中唯一有数字的一条（TTS TTFB 1.22s）超出标准约 4.1 倍。

**判定：NO-GO（维持）**

三条判定依据：

1. 商用门禁不可判 —— TTFP / TTS TTFB / barge-in 三条中，TTS TTFB 实测 1.22s（标准 P50 ≤ 300ms），TTFP 与 barge-in **无任何样本**。
2. 真机证据缺失 —— 09-07 当天 `adb connect 100.75.48.99:46813` → `10061 主动拒绝`（手机无线调试未开），PCM、声学、逐帧对账全部取不到。
3. 下行仍有 8.9s 无法归因 —— Qwen TTFB 1.22s → TTFA 1.25s → 手机 firstAudioFrame 10.1s，中间约 8.9s 无埋点覆盖（手机侧 L5 未接入）。
...
- **影响**：本轮 10.1s 首包中约 8.9s 落在 TTFA 之后到手机播放这一段，**至今无法归因**。
...
- **编号说明（需 lead 确认）**：团队任务板查不到 #13（可能已删或属另一套编号）。本条按派工口径理解为「backend / rtc_bridge 存活缺乏持续监控面」，下列现状均为可坐实事实。
- **现状**：
...
| Qwen TTFB 1.22s / TTFA 1.25s | 派工简报 | | ● | ● | |
| RMS 放行率 4.7% | 派工简报 | | ● | ● | |
| 采集增益 5.9× / 8.3× | 派工简报 | | ● | ● | |
